		...
		nsp.on('connection', function(socket){
            ...
            socket.on('connect:room', function (data, cb) {
                ...
                socket.join(data.room, function (err) {
                	...
                	if (err)
                    	console.log("Cannot connect to the main room. See logs details " + err);
				...
				socket.join(data.username, function (err) {
					console.log(socket.rooms);
					if (err)
						console.log("Cannot connect to the self main room. See logs details " + err);
				});
                ...
                if (isThere === false) {
                    ...
                    for (let i=0; i < rooms.length; i++) {
                        ...
                        rooms[i].socket.broadcast.to(rooms[i].room).emit('user:connect', {
                            ...
                            text: 'has connected the main room.' + data.room,
                            room: data.room
                        });
                    }
                }
				...
				if (data.disconnect) {
					for (let i=0; i < rooms.length; i++) {
						...
						rooms[i].socket.broadcast.to(rooms[i].room).emit('user:disconnect', {
							username: data.username,
							text: 'has left the main room.' + data.room,
							room: data.room
						});
					}
					// remove user's rooms
			...
			socket.on('send:message', function (data) {
				console.log('send:message: ' + data.text + ' from: ' + data.username + ' main room: ' + data.room);
			...
			socket.on('send:private:message', function (data) {
				console.log('send:message: ' + data.text + ' from: ' + data.username + ' main room: ' + data.room + ' to: ' + data.to);
				//io.sockets.emit('send:message', {   <--- send to everybody
